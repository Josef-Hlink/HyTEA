...
def test(args: argparse.Namespace) -> None:
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer,
        step_size=blueprint.optimizer.lr_step.default,
        gamma=blueprint.optimizer.lr_decay.default,
    )
    agent = Agent(
        model=model,
        optimizer=optimizer,
...
def add_test_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    parser.add_argument(
        '--env', dest='env_name', type=str, default='LunarLander-v2', help='The environment to use.'
    )
    parser.add_argument(
        '--train',
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='The number of individual runs to average over.',
    )
    parser.add_argument(
        '-W',
        '--use-wandb',
    ...
    parser.add_argument(
        ...
        type=str,
        default='test',
        help='The name of the wandb team group.',
    )
    return parser
