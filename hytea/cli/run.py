...
def run(args: argparse.Namespace) -> None:
    ...
    cfg = DotDict.from_dict(vars(args))

    assert args.population_size % 2 == 0, 'Population size must be even.'

    args.project_name = create_project_name(cfg)
    ...
    ff = FitnessFunction(cfg, bs)
    ea = EvolutionaryAlgorithm(cfg, ff)
    ea.run()
...
def add_run_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help='The environment to train the agent in.',
    )
    parser.add_argument(
        '--gen',
        ...
        type=int,
        default=5,
        help='The number of generations to run the EA for.',
    )
    parser.add_argument(
        '--pop', dest='population_size', type=int, default=4, help='The size of an EA population.'
    )
    parser.add_argument(
        '--train',
        dest='num_train_episodes',
        type=int,
        default=1000,
    ...
    parser.add_argument(
        '--test',
        dest='num_test_episodes',
        type=int,
        default=100,
        help='The number of episodes to test the agent for.',
    )
    parser.add_argument(
        '--runs',
        dest='num_runs',
        type=int,
        default=3,
        help='The number of runs to average the reward over.',
    )
    parser.add_argument(
        '--wt', dest='wandb_team', type=str, default='hytea', help='The name of the wandb team.'
    )
