...
class EvolutionaryAlgorithm:
    ...
    def initialize_population(self) -> None:
        ...
        self.population = np.random.randint(
            2, size=(self.population_size, self.candidate_size), dtype=np.uint8
        )
        ...
        with Pool() as pool:
            return np.array(
                pool.starmap(
                    self.evaluate,
                    [
                        (candidate, group_name, create_job_type_name(candidate))
    ...
    def crossover(self, selected_candidates: np.ndarray) -> np.ndarray:
        ...
        crossovered_candidates = np.empty(
            (self.lambda_, selected_candidates.shape[1]), dtype=np.uint8
        )
        ...
        for i in range(0, self.lambda_):
            ...
            crossovered_candidates[i] = np.concatenate(
